...
const uint32_t lora_time_interval = 30 * OS_TICKS_PER_SEC;
...
static void lora_event_callback(struct os_event* event) {
    console_printf("-----lora callback\n");

    // TODO Transmit counter
    
    console_printf("Sending ping\n");
    
    buffer[0] = tokens << 8;
    buffer[1] = tokens & 0xFF;

    lora_send(&buffer[0], 2);
